...
import { 
  ...
  TrendingUp, Bell, Sparkles, Monitor, LogOut
} from 'lucide-react';

const Dashboard = ({ setActiveModule, handleLogout }: { setActiveModule: (m: string) => void, handleLogout: () => void }) => {
  ...
  useEffect(() => {
    const quotes = [
      ...
      { text: "LA LOGÍSTICA ES EL CORAZÓN DE CADA EVENTO MEMORABLE.", author: "EQUIPO VENUE" },
      { text: "SERVICIOS QUE SUPERAN LAS EXPECTATIVAS.", author: "FILOSOFÍA CORE" }
    ];
    ...
    <div className="space-y-6 animate-fade-in relative pb-10 no-scrollbar">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 lg:grid-cols-7 gap-3">
        {weekDays.map(day => {
          ...
          return (
            <div key={day.toString()} className={`bg-white rounded-[2.5rem] border min-h-[220px] flex flex-col transition-all overflow-hidden ${isToday ? 'border-blue-500 ring-4 ring-blue-50 shadow-xl scale-[1.02] z-10' : 'border-slate-100 shadow-sm'} ${isCritical ? 'ring-4 ring-red-50 border-red-200' : ''}`}>
