...
import { Search, Wallet, Printer, Calendar, ArrowRight, User, Landmark } from 'lucide-react';
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-20 no-scrollbar">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-white p-6 rounded-[2.5rem] border shadow-sm gap-4">
        <div>
          <h2 className="text-2xl font-black uppercase text-slate-800 tracking-tighter">CARTERA / CUENTAS POR COBRAR</h2>
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">SITUACIÓN FINANCIERA DE CRÉDITOS</p>
        </div>
        <div className="flex flex-wrap items-center gap-6">
           <div className="text-right">
              <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">PENDIENTE TOTAL</p>
              <p className="text-2xl font-black text-amber-600 font-mono tracking-tighter">${totalPortfolio.toFixed(2)}</p>
           </div>
           <div className="relative w-full md:w-64">
            <input type="text" value={search} onChange={e => setSearch(e.target.value.toUpperCase())} placeholder="ID O CLIENTE..." className="w-full p-4 bg-slate-50 border-2 rounded-2xl text-[10px] font-black uppercase pr-12 outline-none shadow-inner" />
          ...
          <div key={o.id} className="bg-white rounded-[2.5rem] border shadow-sm p-6 space-y-5 hover:shadow-xl transition-all border-l-[12px] border-amber-500 group">
            <div className="flex justify-between items-start">
              <div className="space-y-1">
                <p className="text-[8px] font-black text-blue-600 font-mono tracking-widest leading-none">{o.id}</p>
                <h4 className="font-black text-slate-800 uppercase text-xs truncate max-w-[180px] leading-tight mt-1">{o.clientName}</h4>
              </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 shadow-inner">
                <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1">VENTA TOTAL</p>
                <p className="text-xs font-black text-slate-700">${o.total.toFixed(2)}</p>
              </div>
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 shadow-inner">
                <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1">DEUDA ACTUAL</p>
            ...
            <div className="flex items-center gap-2 pt-2 text-[9px] font-black text-slate-400 uppercase">
              <Calendar size={12}/> VENCE: {new Date(o.eventDateEnd).toLocaleDateString()}
            </div>
            ...
            <button onClick={() => setSelectedOrder(o)} className="w-full py-4 shimmer-bg text-white rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-lg flex items-center justify-center gap-3 active:scale-95 transition-all">
              <Wallet size={16}/> GESTIONAR COBRO <ArrowRight size={14}/>
            </button>
          </div>
        ))}
        {filtered.length === 0 && (
          <div className="col-span-full py-24 text-center border-2 border-dashed rounded-[3rem] text-slate-300 font-black uppercase text-[10px] tracking-[0.4em]">SIN PENDIENTES EN CARTERA</div>
        )}
      </div>
    </div>
  );
