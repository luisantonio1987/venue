...
const PendingsModule = () => {
  ...
  const executeResolution = async () => {
    ...
    if (entries.length === 0) {
      setModal({ isOpen: true, type: 'warning', title: 'SIN SELECCIÓN', message: 'INDICA CANTIDADES A RESOLVER.' });
      return;
    }
    ...
    if (type === 'PAID') {
      ...
      setResolutionFlow({ ...resolutionFlow, isOpen: false });
      setModal({ isOpen: true, type: 'success', title: 'STOCK ACTUALIZADO', message: 'ARTÍCULOS REINTEGRADOS CORRECTAMENTE.', autoClose: 1500 });
    }
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-20 no-scrollbar relative">
      <Modal isOpen={modal.isOpen} type={modal.type} title={modal.title} message={modal.message} onConfirm={modal.onConfirm} autoClose={modal.autoClose} onClose={() => setModal({ ...modal, isOpen: false })} />
      ...
      {activePaymentOrder && (
        <PaymentForm 
          ...
          onSaved={async () => {
            ...
            setModal({ isOpen: true, type: 'success', title: 'COBRO EXITOSO', message: 'NOVEDAD LIQUIDADA Y ARCHIVADA.', autoClose: 1500 });
          }} 
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-xl p-8 space-y-6 animate-scale-in border-t-[12px] border-emerald-500 flex flex-col max-h-[90vh]">
            <div className="flex justify-between items-center border-b pb-4">
              <div>
                <h3 className="text-sm font-black text-slate-800 uppercase tracking-widest leading-none">{resolutionFlow.type === 'PAID' ? 'LIQUIDAR FALTANTES' : 'REINTEGRAR ÍTEMS'}</h3>
                <p className="text-[10px] font-black text-slate-400 uppercase mt-2">ORDEN: {resolutionFlow.order.id}</p>
              </div>
              <button onClick={() => setResolutionFlow({ ...resolutionFlow, isOpen: false })} className="text-slate-300 hover:text-red-500 transition-all"><X size={24}/></button>
            </div>

            <div className="flex-1 overflow-y-auto no-scrollbar py-2 space-y-4">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">CANTIDAD A {resolutionFlow.type === 'PAID' ? 'COBRAR' : 'REINTEGRAR'}:</p>
                ...
                {resolutionFlow.order.noveltyItems?.map(item => (
                  <div key={item.productId} className="p-4 flex items-center justify-between group hover:bg-white transition-all">
                    <div className="space-y-1">
                      <p className="text-[11px] font-black uppercase text-slate-800 leading-tight">{item.name}</p>
                      <p className="text-[9px] font-black text-slate-400 uppercase">PENDIENTE: {item.quantityAffected} UDS.</p>
                      {resolutionFlow.type === 'PAID' && <p className="text-[10px] font-black text-emerald-600 font-mono">REPOSICIÓN: ${item.replacementPrice.toFixed(2)}</p>}
                    </div>
                    <input 
                      type="number"
                      min="0"
                      ...
                      onChange={e => {
                        ...
                        setResolutionFlow({ ...resolutionFlow, selectedItems: { ...resolutionFlow.selectedItems, [item.productId]: val } });
                      }}
            ...
            <div className="pt-4 border-t flex flex-col gap-4 shrink-0">
               {resolutionFlow.type === 'PAID' && (
                 <div className="bg-emerald-50 p-4 rounded-2xl flex justify-between items-center border-2 border-emerald-100 shadow-inner">
                    <span className="text-[11px] font-black text-emerald-800 uppercase tracking-widest flex items-center gap-2"><Calculator size={16}/> LIQUIDACIÓN:</span>
                    <span className="text-2xl font-black text-emerald-600 tracking-tighter">${calculateTotalToPay().toFixed(2)}</span>
                 </div>
               )}
               <button onClick={executeResolution} className={`w-full py-5 rounded-[1.8rem] font-black uppercase text-xs shadow-xl flex items-center justify-center gap-2 active:scale-95 transition-all ${resolutionFlow.type === 'PAID' ? 'bg-emerald-600 text-white hover:bg-emerald-700' : 'bg-slate-900 text-white hover:bg-black'}`}>
                 <Check size={20}/> {resolutionFlow.type === 'PAID' ? 'PROCEDER AL COBRO' : 'CONFIRMAR REPOSICIÓN'}
               </button>
            </div>
          </div>
        </div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-white p-6 rounded-[2.5rem] border shadow-sm gap-4 sticky top-0 z-40">
        <h2 className="text-2xl font-black uppercase text-slate-800 tracking-tighter leading-none">PENDIENTES NOVEDADES</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filtered.map(o => (
          <div key={o.id} className="bg-white rounded-[2.5rem] border shadow-sm flex flex-col hover:shadow-xl transition-all border-l-[12px] border-amber-500 overflow-hidden group">
            <div className="p-7 space-y-4">
              ...
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 shadow-inner">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-[9px] font-bold text-slate-400 italic mt-3 border-t pt-2 uppercase">"{o.nuisances || 'SIN OBS'}"</p>
                </div>
              </div>
              {/* Regla 86: Botones visibles */}
              <div className="grid grid-cols-2 gap-3 pt-2 action-button-container">
                <button onClick={() => openResolution(o, 'PAID')} className="py-4 bg-emerald-600 text-white rounded-2xl font-black uppercase text-[10px] shadow-lg flex items-center justify-center gap-2 active:scale-95"><Coins size={16}/> COBRAR</button>
                <button onClick={() => openResolution(o, 'REPLACED')} className="py-4 bg-slate-900 text-white rounded-2xl font-black uppercase text-[10px] shadow-lg flex items-center justify-center gap-2 active:scale-95"><RefreshCw size={16}/> REPONER</button>
              </div>
            </div>
          </div>
        ))}
