...
const PettyCashModule = () => {
  ...
  const handleSave = async () => {
    if (!formData.amount || !formData.reason) {
      setModal({ isOpen: true, type: 'warning', title: 'CAMPOS VACÍOS', message: 'VALOR Y MOTIVO SON OBLIGATORIOS.' });
      return;
    }
    ...
    setModal({ isOpen: true, type: 'success', title: 'REGISTRO EXITOSO', message: `VALE ${sequentialId} GUARDADO EN EGRESOS.` });
  };
    ...
    <div className="space-y-6 animate-fade-in pb-20 no-scrollbar">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-white p-6 rounded-[2.5rem] border shadow-sm gap-4">
        <div className="flex items-center gap-4">
           <div className="p-4 bg-red-50 text-red-600 rounded-3xl shadow-inner"><Banknote size={24}/></div>
           <div>
              <h2 className="text-2xl font-black uppercase text-slate-800 tracking-tighter leading-none">CAJA CHICA / GASTOS</h2>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">VALES DE EGRESO OPERATIVO</p>
           </div>
        </div>
        <div className="flex flex-wrap gap-3 w-full md:w-auto">
          <div className="relative flex-1 md:w-64">
            <input type="text" value={search} onChange={e => setSearch(e.target.value.toUpperCase())} placeholder="FILTRAR..." className="w-full p-4 bg-slate-50 border-2 rounded-2xl text-[10px] font-black uppercase pr-12 outline-none focus:border-red-500 shadow-inner" />
            <Search size={18} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-300" />
          </div>
          <button onClick={() => setIsFormOpen(true)} className="px-8 py-4 bg-red-600 text-white rounded-2xl font-black uppercase text-[10px] flex items-center gap-2 shadow-xl hover:bg-red-700 active:scale-95 transition-all"><Plus size={16}/> NUEVO GASTO</button>
        </div>
      </div>
        ...
        {filtered.map(e => (
          <div key={e.id} className="bg-white rounded-[2rem] border shadow-sm p-6 space-y-4 border-l-[12px] border-red-500 hover:shadow-lg transition-all group">
            ...
            <div className="flex justify-between items-end border-t pt-4 border-slate-50">
               <div><p className="text-[7px] font-black text-slate-300 uppercase">VALOR</p><p className="text-xl font-black text-red-600 font-mono">-${e.amount.toFixed(2)}</p></div>
               <button onClick={() => {
                 setModal({ isOpen: true, type: 'danger', title: 'ELIMINAR VALE', message: '¿CONFIRMA ELIMINAR EL REGISTRO DE GASTO?', onConfirm: () => dbService.delete('cash', e.id) });
               }} className="p-3 text-slate-200 hover:text-red-600 transition-all"><Trash2 size={16}/></button>
            </div>
          </div>
        ))}
      </div>
        ...
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in overflow-y-auto no-scrollbar">
          <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-lg p-10 space-y-8 animate-scale-in my-auto border-t-[16px] border-red-600">
            <div className="flex justify-between items-center border-b pb-4">
              <h3 className="font-black text-slate-800 uppercase text-xs tracking-widest">VALE DE CAJA CHICA</h3>
              <button onClick={() => setIsFormOpen(false)} className="text-slate-400 hover:text-red-600 transition-all"><X size={24} /></button>
            </div>
            <div className="space-y-6">
              <div><label className="text-[9px] font-black text-slate-400 uppercase ml-1 block mb-2 tracking-widest">MONTO ($)</label><input type="text" value={formData.amount} onChange={e => setFormData({...formData, amount: e.target.value})} className="w-full p-5 bg-slate-50 border-2 rounded-2xl font-black text-3xl text-red-600 text-center outline-none focus:border-red-500 shadow-inner" placeholder="0.00" /></div>
              <div><label className="text-[9px] font-black text-slate-400 uppercase ml-1 block mb-2 tracking-widest">CONCEPTO / MOTIVO</label><textarea value={formData.reason} onChange={e => setFormData({...formData, reason: e.target.value})} className="w-full p-4 bg-slate-50 border-2 rounded-2xl text-[11px] font-bold min-h-[100px] outline-none shadow-inner uppercase" placeholder="DESCRIPCIÓN DEL GASTO..." /></div>
              <div><label className="text-[9px] font-black text-slate-400 uppercase ml-1 block mb-2 tracking-widest">BENEFICIARIO</label><input type="text" value={formData.beneficiary} onChange={e => setFormData({...formData, beneficiary: e.target.value})} className="w-full p-4 bg-slate-50 border-2 rounded-2xl text-[10px] font-black uppercase outline-none shadow-inner" placeholder="NOMBRE / EMPRESA" /></div>
            </div>
            <button onClick={handleSave} className="w-full py-6 bg-red-600 text-white rounded-[2rem] font-black uppercase text-[11px] shadow-2xl flex items-center justify-center gap-4 active:scale-95 transition-all">
              <Check size={20} strokeWidth={3} /> CONFIRMAR Y REGISTRAR
            </button>
          </div>
        </div>
      )}
