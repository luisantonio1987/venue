...
import { Order, CompanyData, UserAccount, Client } from '../types';
import Modal, { ModalType } from '../components/Modal';
import { 
  Edit3, Trash2, Package, Search, Calendar, Printer, Clock, FileDigit, X, Truck, ArrowRight, Wallet, CheckSquare, Square, FileText
} from 'lucide-react';
...
const ConfirmedOrders = ({ onEdit, company, user }: { onEdit: (id: string) => void, company: CompanyData | null, user: UserAccount }) => {
  ...
  const handleDispatch = (o: Order) => {
    ...
    setModal({
      isOpen: true,
      type: 'confirm',
      title: 'PROCEDER CON EL DESPACHO',
      message: `EL PEDIDO ${o.id} PASARÁ A LOGÍSTICA. EL EB N° ES OPCIONAL. ¿CONFIRMA?`,
      onConfirm: async () => {
        await dbService.update('orders', o.id, { status: 'EN_PROCESO' });
        setModal({ isOpen: true, type: 'success', title: 'OPERACIÓN EXITOSA', message: 'ORDEN ENVIADA A DESPACHOS.' });
      }
    });
  };
  ...
  const handleBulkDelete = () => {
    setModal({
      ...
      message: `ESTÁ POR ELIMINAR ${selectedIds.length} PEDIDOS. ¿CONFIRMA ESTA ACCIÓN?`,
      onConfirm: async () => {
  ...
  const formatDateRange = (start: number, end: number) => {
    const s = new Date(start).toLocaleDateString('es-EC', { day: '2-digit', month: '2-digit', year: 'numeric' });
    const e = new Date(end).toLocaleDateString('es-EC', { day: '2-digit', month: '2-digit', year: 'numeric' });
    return s === e ? s : `${s} AL ${e}`;
  };
    ...
    <div className="space-y-6 animate-fade-in pb-24 no-scrollbar">
        ...
        <div className="fixed inset-0 z-[500] bg-white overflow-auto p-10 animate-fade-in print:p-0">
           <div className="max-w-4xl mx-auto">
             <div className="flex justify-end gap-3 mb-10 print:hidden">
               <button onClick={() => window.print()} className="px-8 py-3 bg-slate-900 text-white rounded-xl font-black uppercase text-[10px] shadow-xl">IMPRIMIR GUÍA</button>
             ...
             <DeliveryGuideTemplate 
               order={printingOrder} 
               company={company} 
               client={dbService.getAll('clients').find((c:Client) => c.id === printingOrder.clientId)} 
               user={user} 
             />
           </div>
        </div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-white p-6 rounded-[2.5rem] border shadow-sm gap-4">
        <div className="flex items-center gap-4">
          <h2 className="text-2xl font-black uppercase text-slate-800 tracking-tighter">CONTROL DE PEDIDOS</h2>
          {selectedIds.length > 0 && <button onClick={handleBulkDelete} className="p-3 bg-red-50 text-red-600 rounded-xl hover:bg-red-600 hover:text-white transition-all shadow-sm" title="ELIMINAR SELECCIÓN"><Trash2 size={20}/></button>}
        </div>
        <div className="relative w-full md:w-80">
          <input type="text" value={search} onChange={e => setSearch(e.target.value.toUpperCase())} placeholder="BUSCAR PEDIDO..." className="w-full p-4 bg-slate-50 border-2 rounded-2xl text-[10px] font-black uppercase pr-12 outline-none focus:border-blue-500 transition-all shadow-inner" />
          ...
          <div key={o.id} className={`bg-white rounded-[2rem] border shadow-sm p-5 space-y-4 hover:shadow-lg transition-all relative overflow-hidden group ${selectedIds.includes(o.id) ? 'ring-4 ring-blue-500' : ''}`}>
            ...
            <div className="flex justify-between items-start pr-8">
              <div className="space-y-1">
                ...
                <p className="text-[8px] font-black text-slate-400 font-mono tracking-widest">{o.id}</p>
              </div>
              <button onClick={() => setActivePaymentOrder(o)} className="p-2.5 bg-emerald-50 text-emerald-600 rounded-xl" title="REGISTRAR COBRO"><Wallet size={16}/></button>
            </div>

            <div className="bg-slate-50 rounded-2xl p-4 space-y-3 border border-slate-100">
              <div className="flex flex-col text-[8px] font-black text-slate-400 uppercase">
                <span className="flex items-center gap-1"><Calendar size={10}/> FECHAS DEL EVENTO (REGLA 66):</span>
                <span className="text-slate-800 mt-1 font-mono tracking-tighter">{formatDateRange(o.eventDateStart, o.eventDateEnd)}</span>
              </div>
              <div className="flex flex-col text-[8px] font-black text-slate-300 uppercase border-t pt-2 opacity-60">
                <span className="flex items-center gap-1"><Clock size={10}/> FECHA PEDIDO (INFORMATIVO):</span>
                <span className="mt-1">{new Date(o.orderDate).toLocaleDateString('es-EC')}</span>
              </div>
              <div className="space-y-1.5 pt-2 border-t mt-1">
                ...
                <input type="text" value={o.ebNumber || ''} placeholder="0000" onChange={e => dbService.update('orders', o.id, { ebNumber: e.target.value.replace(/\D/g,'') })} className="w-full p-2 bg-white border rounded-xl text-center font-black text-xs text-blue-600 outline-none shadow-inner" />
              </div>
            </div>
            ...
            {o.status === 'PROFORMA' ? (
              ...
              isDispatchAvailable(o.eventDateStart) ? (
                <button onClick={() => handleDispatch(o)} className="w-full py-3 shimmer-bg text-white rounded-xl font-black uppercase text-[9px] tracking-widest shadow-lg flex items-center justify-center gap-2 animate-pulse"><Package size={14}/> DESPACHAR ORDEN</button>
              ) : (
                <div className="w-full py-3 bg-slate-50 text-slate-300 rounded-xl font-black uppercase text-[8px] flex items-center justify-center gap-2 border-2 border-dashed"><Clock size={12}/> DISPONIBLE EN 3 DÍAS</div>
              )
            )}
          </div>
        ))}
