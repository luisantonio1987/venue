...
import { 
  Edit3, Trash2, Package, Search, Calendar, Printer, Clock, FileDigit, Truck, ArrowRight, Wallet, CheckSquare, Square, FileText
} from 'lucide-react';
...
const ConfirmedOrders = ({ onEdit, company, user }: { onEdit: (id: string) => void, company: CompanyData | null, user: UserAccount }) => {
  ...
  const handleDispatch = (o: Order) => {
    ...
    setModal({
      ...
      message: `EL PEDIDO ${o.id} PASARÁ A LOGÍSTICA. ¿CONFIRMA ESTA ACCIÓN?`,
      onConfirm: async () => {
    ...
    <div className="space-y-6 animate-fade-in pb-24 no-scrollbar">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-white p-6 rounded-[2.5rem] border shadow-sm gap-4">
        <div className="flex items-center gap-4">
          <h2 className="text-2xl font-black uppercase text-slate-800 tracking-tighter leading-none">PEDIDOS CONFIRMADOS</h2>
        ...
        {filtered.map(o => (
          <div key={o.id} className={`bg-white rounded-[2rem] border shadow-sm p-5 space-y-4 hover:shadow-lg transition-all relative overflow-hidden group ${selectedIds.includes(o.id) ? 'ring-4 ring-blue-500' : ''}`}>
            ...
            <div className="flex justify-between items-start pr-8">
              ...
              <button onClick={() => setActivePaymentOrder(o)} className="p-2.5 bg-emerald-50 text-emerald-600 rounded-xl shadow-sm hover:bg-emerald-600 hover:text-white transition-all" title="Registrar cobro"><Wallet size={16}/></button>
            </div>

            <div className="bg-slate-50 rounded-2xl p-4 space-y-3 border border-slate-100 shadow-inner">
              <div className="flex flex-col text-[8px] font-black text-slate-400 uppercase">
                <span className="flex items-center gap-1"><Calendar size={10}/> FECHAS EVENTO (REGLA 66):</span>
            ...
            <div className="grid grid-cols-3 gap-2">
                <button onClick={() => onEdit(o.id)} className="p-3 bg-slate-50 text-blue-600 rounded-xl hover:bg-blue-600 hover:text-white transition-all shadow-sm" title="Editar pedido"><Edit3 size={14}/></button>
                ...
                <button onClick={() => dbService.update('orders', o.id, { status: 'ANULADO' })} className="p-3 bg-slate-50 text-red-600 rounded-xl hover:bg-red-600 hover:text-white transition-all shadow-sm" title="Anular pedido"><Trash2 size={14}/></button>
            </div>
