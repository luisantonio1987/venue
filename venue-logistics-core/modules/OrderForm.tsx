...
const OrderForm: React.FC<OrderFormProps> = ({ editOrderId, onSaved, onCancel, company, user }) => {
  ...
  useEffect(() => {
    ...
    if (editOrderId) {
      ...
      if (o) {
        ...
        setSelectedClient(dbService.getAll('clients').find((c: Client) => c.id === o.clientId) || null);
        setItems(o.items || []);
        setDates({
  ...
  const discountableItemsSubtotal = items.filter(i => 
    !toUpper(i.name).includes('TRANSPORTE') && 
    !toUpper(i.name).includes('SALONERO')
  ).reduce((sum, i) => sum + (i.price * i.quantity), 0) * rentalDays;

  const dVal = parseNum(discountValue);
  const discountAmount = discountType === 'PERCENTAGE' ? (discountableItemsSubtotal * (dVal / 100)) : dVal;
    ...
    setModal({
      ...
      onConfirm: async () => {
        ...
        if (editOrderId) {
          ...
          // Notificación Push (Regla 49)
          await dbService.sendNotification("Nuevo Pedido", `Se ha registrado ${activeTab}: ${sequentialId} para ${selectedClient.name}`, "ORDER");
          
          setModal({
            isOpen: true,
            type: 'persuade',
            title: 'REGISTRO EXITOSO',
            message: 'EL DOCUMENTO HA SIDO GUARDADO. ¿DESEA REGISTRAR EL PAGO AHORA O GESTIONAR COMO CRÉDITO?',
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-white p-5 rounded-[2.5rem] border shadow-sm gap-4">
        <div className="flex items-center gap-4">
          ...
          <div className="flex bg-slate-100 p-1 rounded-2xl border shadow-inner">
            <button onClick={() => setActiveTab('SALE')} className={`px-8 py-2.5 rounded-xl text-[10px] font-black uppercase transition-all ${activeTab === 'SALE' ? 'bg-white text-blue-600 shadow-md animate-pulse-latent' : 'text-slate-400'}`}>MODULO VENTA</button>
            <button onClick={() => setActiveTab('PROFORMA')} className={`px-8 py-2.5 rounded-xl text-[10px] font-black uppercase transition-all ${activeTab === 'PROFORMA' ? 'bg-white text-amber-600 shadow-md animate-pulse-latent' : 'text-slate-400'}`}>MODULO PROFORMA</button>
          </div>
        </div>
        <button onClick={processSave} className="px-10 py-4 shimmer-bg text-white rounded-2xl font-black uppercase text-[10px] flex items-center gap-3 shadow-xl active:scale-95 transition-all">
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-white p-8 rounded-[3rem] border shadow-sm space-y-6 sticky top-24 max-h-[85vh] overflow-y-auto no-scrollbar flex flex-col">
            ...
            <div className="pt-8 border-t-4 border-dashed border-slate-100 space-y-4 shrink-0">
              <div className="flex justify-between text-[10px] font-black text-slate-400 uppercase tracking-widest"><span>TOTAL BRUTO</span><span>${subtotalBruto.toFixed(2)}</span></div>
              <div className="flex justify-between text-[10px] font-black text-indigo-600 uppercase tracking-widest"><span>IVA GENERADO</span><span>+${tax.toFixed(2)}</span></div>
              {hasTransport && <div className="flex justify-between text-[10px] font-black text-blue-600 uppercase tracking-widest"><span>TRANSPORTE</span><span>+${tVal.toFixed(2)}</span></div>}
