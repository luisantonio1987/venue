...
import { X, Check, ShoppingCart, Tag, Truck, Receipt, UserPlus, Trash2, Calculator } from 'lucide-react';
import Modal, { ModalType } from '../components/Modal';
import PaymentForm from '../components/PaymentForm';

const SalesModule = ({ onSaved, company, user }: { onSaved: () => void; company: CompanyData | null; user: UserAccount }) => {
  ...
  const processSave = async () => {
    if (!selectedClient || items.length === 0) {
      setModal({ isOpen: true, type: 'warning', title: 'DATOS FALTANTES', message: 'VINCULE UN CLIENTE Y AGREGUE ARTÍCULOS.' });
      return;
    }
    
    setModal({
      isOpen: true, type: 'confirm', title: 'CONFIRMAR REGISTRO',
      message: `¿GUARDAR ESTA ${activeTab === 'SALE' ? 'VENTA' : 'PROFORMA'} POR $${finalTotal.toFixed(2)}?`,
      onConfirm: async () => {
        const prefix = activeTab === 'PROFORMA' ? 'PR' : 'VE';
        const seqId = await dbService.generateSequentialId(prefix);
        const orderData: Order = {
          id: seqId, clientId: selectedClient.id, clientName: selectedClient.name,
          status: activeTab === 'PROFORMA' ? 'PROFORMA' : 'CONFIRMADA',
          orderDate: Date.now(), eventDateStart: new Date(dates.start).getTime(),
          eventDateEnd: new Date(dates.end).getTime(), rentalDays, items, 
          hasTransport, transportValue: tVal, deliveryAddress: toUpper(deliveryAddress), 
          discountType, discountValue: dVal, subtotal: subtotalBrutoItems, 
          tax, total: finalTotal, paidAmount: 0, 
          logisticsType: hasTransport ? 'CON_TRANSPORTE' : 'SIN_TRANSPORTE',
          payments: [], createdBy: user.name
        };

        await dbService.add('orders', orderData);
        setSavedOrderRef(orderData);
        await dbService.sendNotification("NUEVO PEDIDO", `${seqId} PARA ${selectedClient.name}`, "ORDER");

        setModal({
          isOpen: true, type: 'persuade', title: 'REGISTRO GUARDADO',
          message: 'EL DOCUMENTO SE GENERÓ CON ÉXITO. ¿PROCEDER AL COBRO O VENTA A CRÉDITO?',
          onConfirm: () => setShowPayment(true),
          onCancelAction: () => onSaved(),
          confirmLabel: 'REGISTRAR PAGO', cancelLabel: 'VENTA A CRÉDITO'
        });
      }
    });
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-20 no-scrollbar overflow-x-hidden">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center bg-white p-5 rounded-[2.5rem] border shadow-sm gap-4">
        <div className="flex bg-slate-100 p-1.5 rounded-2xl border shadow-inner w-full md:w-auto">
          {/* Regla 1: Pulsación latente en pestaña activa */}
          <button onClick={() => setActiveTab('SALE')} className={`flex-1 md:flex-none px-12 py-3 rounded-xl text-[10px] font-black uppercase transition-all duration-500 ${activeTab === 'SALE' ? 'bg-white text-blue-600 shadow-md animate-pulse-latent' : 'text-slate-400'}`}>VENTAS</button>
          <button onClick={() => setActiveTab('PROFORMA')} className={`flex-1 md:flex-none px-12 py-3 rounded-xl text-[10px] font-black uppercase transition-all duration-500 ${activeTab === 'PROFORMA' ? 'bg-white text-amber-600 shadow-md animate-pulse-latent' : 'text-slate-400'}`}>PROFORMAS</button>
        </div>
        <button onClick={processSave} className="w-full md:w-auto px-10 py-4 shimmer-bg text-white rounded-2xl font-black uppercase text-[10px] flex items-center justify-center gap-3 shadow-xl active:scale-95 transition-all"><Check size={18}/> GUARDAR {activeTab === 'SALE' ? 'VENTA' : 'PROFORMA'}</button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="bg-white p-8 rounded-[3rem] border shadow-sm space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="relative">
                ...
                <div className="flex gap-2">
                  <div className="relative flex-1">
                    <input type="text" value={selectedClient ? selectedClient.name : searchClient} onChange={e => { setSearchClient(toUpper(e.target.value)); setSelectedClient(null); setIsClientListOpen(true); }} className="w-full p-4 bg-slate-50 border-2 rounded-2xl font-black text-xs uppercase outline-none focus:border-blue-500 shadow-inner" placeholder="BUSCAR..." />
                    {isClientListOpen && !selectedClient && (
                      <div className="absolute top-full left-0 right-0 mt-2 bg-white border rounded-2xl shadow-2xl z-[100] max-h-48 overflow-y-auto no-scrollbar">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div><label className="text-[10px] font-black text-slate-400 uppercase ml-1 mb-2 block">FECHA FIN</label><input type="date" value={dates.end} min={dates.start} onChange={e => setDates({...dates, end: e.target.value})} className="w-full p-4 bg-slate-50 border-2 rounded-2xl font-black text-xs outline-none focus:border-blue-500 shadow-inner" /></div>
              </div>
            </div>

            <div className="space-y-4">
              <div className="flex justify-between items-center border-b pb-4">
                <h3 className="text-[11px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-3"><ShoppingCart size={18} className="text-blue-600"/> DETALLE DE PEDIDO</h3>
                <span className="bg-blue-50 text-blue-600 px-5 py-2 rounded-full text-[10px] font-black uppercase tracking-tighter">{rentalDays} DÍA(S)</span>
              </div>
              <select onChange={e => { const p = products.find(x => x.id === e.target.value); if(p) handleAddItem(p); e.target.value = ""; }} className="w-full p-5 bg-slate-900 text-white rounded-3xl font-black text-sm shadow-xl cursor-pointer hover:bg-black transition-all">
                <option value="">＋ AÑADIR AL DETALLE...</option>
              ...
              <div className="border rounded-[2.5rem] overflow-hidden bg-white shadow-inner overflow-x-auto no-scrollbar">
                <table className="w-full text-left min-w-[600px]">
                  <thead className="bg-slate-50 border-b text-[9px] font-black text-slate-400 uppercase tracking-widest">
                    <tr><th className="px-8 py-5">ÍTEM</th><th className="px-4 py-5 text-center">CANT.</th><th className="px-4 py-5 text-center">PRECIO ($)</th><th className="px-8 py-5 text-right">TOTAL {rentalDays} D.</th><th className="w-16"></th></tr>
                  </thead>
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-white p-8 rounded-[3rem] border shadow-sm space-y-6 sticky top-24 max-h-[85vh] overflow-y-auto no-scrollbar flex flex-col">
            <h3 className="text-[12px] font-black uppercase text-slate-800 tracking-[0.2em] flex items-center gap-3 border-b pb-5 shrink-0"><Receipt size={20} className="text-blue-600"/> LIQUIDACIÓN</h3>
            <div className="space-y-4 flex-1">
              <div className={`p-5 rounded-[2rem] border-2 transition-all ${hasTransport ? 'bg-blue-50 border-blue-200 shadow-inner' : 'bg-slate-50 border-transparent opacity-60'}`}>
                <label className="flex items-center gap-3 cursor-pointer">
                  <input type="checkbox" checked={hasTransport} onChange={e => setHasTransport(e.target.checked)} className="w-5 h-5 rounded-lg text-blue-600" />
                ...
                {hasTransport && (
                  <div className="mt-5 space-y-4 animate-scale-in">
                    <input type="text" value={transportValue} onChange={e => setTransportValue(e.target.value)} placeholder="MONTO $" className="w-full p-4 bg-white border-2 rounded-2xl font-black text-xs text-blue-700 outline-none shadow-inner" />
                    <textarea value={deliveryAddress} onChange={e => setDeliveryAddress(toUpper(e.target.value))} placeholder="DIRECCIÓN..." className="w-full p-4 bg-white border-2 rounded-2xl text-[10px] font-bold min-h-[80px] outline-none shadow-inner" />
                  </div>
                )}
              </div>
              <div className={`p-5 rounded-[2rem] border-2 transition-all ${parseNum(discountValue) > 0 ? 'bg-amber-50 border-amber-200 shadow-inner' : 'bg-slate-50 border-transparent opacity-60'}`}>
                <div className="flex justify-between items-center mb-4">
                  <span className="text-[11px] font-black text-amber-800 uppercase flex items-center gap-2"><Tag size={16}/> DESCUENTO (EXCL. TR/SAL)</span>
            ...
            <div className="pt-8 border-t-4 border-dashed border-slate-100 space-y-4 shrink-0">
              <div className="flex justify-between text-[10px] font-black text-slate-400 uppercase tracking-widest"><span>SUBTOTAL</span><span>${subtotalBrutoItems.toFixed(2)}</span></div>
              ...
              <div className="pt-8 border-t-8 border-slate-900 mt-6 relative overflow-hidden rounded-3xl shadow-lg">
                ...
                <div className="flex justify-between items-end relative z-10 px-2 pb-2">
                  <div className="pb-1"><span className="text-[11px] font-black text-slate-400 uppercase tracking-[0.3em] block mb-1">TOTAL</span><p className="text-[9px] font-black text-blue-600 uppercase tracking-widest animate-pulse">NETO FINAL</p></div>
